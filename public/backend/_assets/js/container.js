...
function noteadd(rowid,container_id,container_name,seal_no){
	...
	$.ajax({
      type: "GET",
	  url: urlgt+"admin/container-certno",
...
function deletedid(){
	...
	$.ajax({
      type: "get",
      url: urlgt+"admin/container-delete",
      ...
      beforeSend: function() {
		...
		//$("#row_"+rlidrw).hide();
		setTimeout(function(){ window.location.href = urlgt+"admin/container"; }, 60);
...
var urlgt=arr[0];
    ...
    $.ajax({
      type: "get",
      url: urlgt+"admin/container-data",
...
$('#ContainerNumber').on('input', function() {
	...
	var is_name=input.val();
	if(is_name){
		var mcnm = document.getElementById('ContainerNumber').value;
		if (mcnm.length < 3) {
			...
			$('<label id="nam3" class="text-danger">Please Enter Values Not Less Than 3 Digits</label>').insertAfter('#ValidContainerNo');
			$("#ValidContainerNo").focus;
		} else if ( mcnm.search(/[a-zA-Z]/i) < 0 || mcnm.search(/[0-9]/) < 0) {
			var input=$("#ContainerNumber");
			var urlvl=$("#ContainerNumber").val();
			input.removeClass("valid").addClass("invalid");
			$('#nam3').remove();
			$("#nam3").removeClass("error").addClass("error_show");
		$('<label id="nam3" class="text-danger">Container No Must be Alpha Numeric</label>').insertAfter('#ValidContainerNo');
			$("#ValidContainerNo").focus;
		} else {
			var element=input;
			var error_ueleent=$("#nam3", element.parent());
			error_ueleent.removeClass("error_show").addClass("error");
			input.removeClass("invalid").addClass("valid");
		}
	} else {
...
$('#SealNumber').on('input', function() {
	var input=$(this);
	var is_name=input.val();
	if(is_name){
		var mcnm = document.getElementById('SealNumber').value;
		if (mcnm.length < 3) {
			...
			$('<label id="nam4" class="text-danger">Please Enter Values Not Less Than 3 Digits</label>').insertAfter('#ValidSealNumber');
			...
			$('#nam4').remove();
			$("#nam4").removeClass("error").addClass("error_show");
			$('<label id="nam4" class="text-danger">Seal No Must be Alpha Numeric</label>').insertAfter('#ValidSealNumber');
			$("#ValidSealNumber").focus;
		} else {
			var element=input;
			var error_ueleent=$("#nam4", element.parent());
			error_ueleent.removeClass("error_show").addClass("error");
			input.removeClass("invalid").addClass("valid");
		}
	} else {
...
$('#ContainerNumber').bind('keyup blur',function(){
    var node = $(this);
    node.val(node.val().replace(/[^a-zA-Z0-9@#$\-\/]/gi,'') ); }
);
$('#ContainerNumber').keyup(function(){

	var str = $('#ContainerNumber').val();
	$('#ContainerNumber').val(str.toUpperCase());
	var regex = new RegExp("^[^a-zA-Z0-9@#$\-\/]+$");
	var str = String.fromCharCode(!e.charCode ? e.which : e.charCode);
	if (regex.test(str)) {
		$("#ContainerNumber").removeClass("invalid").addClass("valid");
...
$('#SealNumber').bind('keyup blur',function(){
    var node = $(this);
    node.val(node.val().replace(/[^a-zA-Z0-9@#$\-\/]/gi,'') ); }
);
$('#SealNumber').keyup(function(){
	var str = $('#SealNumber').val();
	var regex = new RegExp("^[a-zA-Z]+$");
	var str = String.fromCharCode(!e.charCode ? e.which : e.charCode);
	if (regex.test(str)) {
		$("#SealNumber").removeClass("invalid").addClass("valid");
...
$(".sv-btn").click(function(event){
	...
	var error_free=true;
		...
		if (urlvl==''){
			...
			$.ajax({
				...
				type: 'POST',
				async: false,
				url: url+"admin/container/seal_number_check",
		...
		if (urlvl==''){
			...
			$.ajax({
				...
				type: 'POST',
				async: false,
				url: url+"admin/container/container_number_check",
		...
		if (urlvl==''){
			...
			$('#nam3').remove();
			$("#nam3").removeClass("error").addClass("error_show");
			$('<label id="nam3" class="text-danger">Container Number is required</label>').insertAfter('#ValidContainerNo');
			$("#ValidContainerNo").focus;
			error_free=false;
		} else {
			var mcnm = document.getElementById('ContainerNumber').value;
		...
		if (urlvl==''){
			...
			$('#nam4').remove();
			$("#nam4").removeClass("error").addClass("error_show");
			$('<label id="nam4" class="text-danger">Seal Number is required</label>').insertAfter('#ValidSealNumber');
			$("#ValidSealNumber").focus;
			error_free=false;
		} else {
			var mcnm = document.getElementById('SealNumber').value;
	...
	$.ajax({
			type: "get",
			url: url+"admin/store-container",
			data: "originport="+encodeURIComponent(originport)+"&destinationport="+encodeURIComponent(destinationport)+'&containernumber='+encodeURIComponent(containernumber)+"&sealnumber="+encodeURIComponent(sealnumber)+"&stuffingdate="+encodeURIComponent(stuffingdate),
			beforeSend: function() {
				showLoader();
			...
			setTimeout(function(){ window.location.href = url+"admin/edit-container/"+datad; /*$(".updv").hide();*/ }, 5);
...
$(".fnsav").click(function(event){
	...
	$.ajax({
		...
		setTimeout(function(){ window.location.href = url+"admin/container"; /*$(".updv").hide();*/ }, 60);
